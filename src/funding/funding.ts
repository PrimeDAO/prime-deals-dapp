...
export class Funding {
  ...
  private daoOtherThanRepresentedByCurrentAccountTokens: ITokenFunding[];
  private daoRepresentedByCurrentAccountTokens: ITokenFunding[];
  ...
  public async bind(): Promise<void> {
    if (!this.deal.daoRepresentedByCurrentAccount) this.verifySecurity();
    ...
    this.daoOtherThanRepresentedByCurrentAccountTokens = JSON.parse(JSON.stringify(this.deal.daoOtherThanRepresentedByCurrentAccount.tokens));
    this.daoOtherThanRepresentedByCurrentAccountTokens.forEach(x => {this.setTokenContractInfo(x, this.deal.daoOtherThanRepresentedByCurrentAccount);});
    ...
    this.daoRepresentedByCurrentAccountTokens = JSON.parse(JSON.stringify(this.deal.daoRepresentedByCurrentAccount.tokens));
    this.daoRepresentedByCurrentAccountTokens.forEach(x => {this.setTokenContractInfo(x, this.deal.daoRepresentedByCurrentAccount);});

    if (this.daoRepresentedByCurrentAccountTokens.length === 1) {
  ...
  private checkMaxAmount(): void {
    if (this.deal.daoRepresentedByCurrentAccount?.tokens.length > 0 && this.selectedToken) {
      const remainingNeeded = (this.daoRepresentedByCurrentAccountTokens[this.selectedToken])?.required;
  ...
  private async depositTokens(): Promise<void> {
    const tokenSymbol = this.daoRepresentedByCurrentAccountTokens[this.selectedToken].symbol;
    ...
    // const token = this.daoRepresentedByCurrentAccountTokens[this.selectedToken] as ITokenFunding;
    ...
    this.eventAggregator.publish("handleInfo", new EventConfig(`Depositing ${BigNumber.from(this.depositAmount)} ${tokenSymbol} on behalf of ${this.deal.daoRepresentedByCurrentAccount.name}`, EventMessageType.Info, "Deposit Submitted"));
  ...
  public setDeposits() : void {
    ...
    //   createdAt: new Date(),
    //   dao: this.deal.daoRepresentedByCurrentAccount,
    //   depositId: 1234,
    //   token: this.deal.daoRepresentedByCurrentAccount.tokens[0],
    //   type: "deposit",
    ...
    //   dao: this.deal.daoRepresentedByCurrentAccount,
    //   depositId: 1234,
    //   token: this.deal.daoOtherThanRepresentedByCurrentAccount.tokens[0],
    //   type: "deposit",
    //   txid: "0xc6539832b952d3e37fcee30984806798bb7bbc737e2b567a40788b942acd6367",
    ...
    // return;
    this.deposits = [...this.mapTransactionsToDeposits(this.deal.daoTokenTransactions.get(this.deal.daoRepresentedByCurrentAccount)), ...this.mapTransactionsToDeposits(this.deal.daoTokenTransactions.get(this.deal.daoOtherThanRepresentedByCurrentAccount))].sort((a, b) => b.createdAt < a.createdAt ? 1 : -1);
  }
  ...
  @computedFrom("daoRepresentedByCurrentAccountTokens")
  public get tokenSelectData() : IPSelectItemConfig[]{
    return this.daoRepresentedByCurrentAccountTokens.map((x, index) => ({
      text: x.symbol,
  ...
  public get tokensToClaim() : IDaoClaimToken[]{
    return this.daoRepresentedByCurrentAccountTokens.map(x => ({
      token: x,
      claimable: 1,
      locked: 2,
    }));
  ...
  public async setAccountBalance() : Promise<void> {
    const contract = this.tokenService.getTokenContract(this.daoRepresentedByCurrentAccountTokens[this.selectedToken].address);
