...
Then("I can see Partnered Deals", () => {
  ...
  cy.get("horizontal-scroller").should("be.visible").children().children().children("deal-summary").should("have.length", 4);
  cy.contains("div", "All partnered deals").should("be.visible");
  cy.get("[data-test='all-deals-grid']").should("be.visible").children("a").should("have.length", 4);
});
